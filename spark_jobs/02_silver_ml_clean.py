...
logger = logging.getLogger("silver_ml_bigquery_native")
ml = "ml"
...
def process_representation(spark: SparkSession, cfg: Dict[str, Any], rep: str) -> None:
    ...
    df = read_bq(spark, cfg, f"bronze_{ml}", in_table)
    sources = [r["source_file"] for r in df.select("source_file").distinct().collect()]
    done = processed_sources(cfg, "silver", ml, rep)
    new_sources = [s for s in sources if s not in done]
    if not new_sources:
        logger.info("No new bronze %s rows for silver %s", rep, ml)
        return
    ...
    write_bq(result, cfg, f"silver_{ml}", out_table, "append")
    for source in new_sources:
        write_audit_status(cfg, "silver", ml, rep, source, "SUCCESS")
    result.unpersist()


def main():
    parser = argparse.ArgumentParser(description="Silver ml clean to BigQuery native")
    parser.add_argument("--config_path", required=True)
    args = parser.parse_args()
    cfg = load_yaml(args.config_path)
    spark = create_spark("silver_ml_bigquery_native")
    try:
