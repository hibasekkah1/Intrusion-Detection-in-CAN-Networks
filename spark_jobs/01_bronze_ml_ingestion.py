...
logger = logging.getLogger("bronze_ml_bigquery_native")
ml = "ml"
...
def process_representation(spark: SparkSession, cfg: Dict[str, Any], rep: str) -> None:
    ...
    done = processed_sources(cfg, "bronze", ml, rep)
    files = [f for f in list_parquet_files(landing) if f["source_file"] not in done]
    if not files:
        logger.info("No new landing %s files for bronze %s", rep, ml)
        return
    for f in files:
        try:
            ...
            write_bq(df, cfg, f"bronze_{ml}", table_name, "append")
            write_audit_status(cfg, "bronze", ml, rep, f["source_file"], "SUCCESS")
        except Exception as e:
            write_audit_status(cfg, "bronze", ml, rep, f["source_file"], "FAILED", error_message=str(e))
            raise


def main():
    parser = argparse.ArgumentParser(description="Bronze ml ingestion to BigQuery native")
    parser.add_argument("--config_path", required=True)
    args = parser.parse_args()
    cfg = load_yaml(args.config_path)
    spark = create_spark("bronze_ml_bigquery_native")
    try:
